...
% CAV
u0=@(s0,v0,s1,v1,v2,v3,beta01,beta03)alpha01*(V0(s1-s0-l)-v0)+...
...
model=@(t,x,xdelay,sL,vL,tau,sigma,beta01,beta03)...
    [x(2);...
     sat(uh(xdelay(1,1),xdelay(2,1),sL(t-tau),vL(t-tau)));...
     x(4);...
     sat(uh(xdelay(3,1),xdelay(4,1),xdelay(1,1),xdelay(2,1)));...
     x(6);...
     sat(u0(xdelay(5,2),xdelay(6,2),xdelay(3,2),xdelay(4,2),xdelay(2,2),vL(t-sigma),beta01,beta03))];
...
[beta01_m,beta03_m]=ndgrid(beta01_v,beta03_v);

% memory allocation
sFollow=zeros(length(beta01_v),length(beta03_v),length(xinit(0))/2,length(time));
...
for kb01=1:length(beta01_v)
    ...
    for kb03=1:length(beta03_v)
        ...
        disp(['Simulation #',num2str((kb01-1)*length(beta03_v)+kb03),'/',...
            num2str(length(beta01_v)*length(beta03_v)),'.']);
        sol=dde23(@(t,x,xdelay)model(t,x,xdelay,sL,vL,tau,sigma,beta01,beta03),...
        ...
        % store results
        sFollow(kb01,kb03,:,:)=svalues;
        vFollow(kb01,kb03,:,:)=vvalues;
        hFollow(kb01,kb03,:,:)=hvalues;
        aFollow(kb01,kb03,:,:)=avalues;
        epsFollow(kb01,kb03,:,:)=epsvalues;
        wFollow(kb01,kb03,:,:)=wvalues;
    end
end
toc;
...
[beta01idx,beta03idx]=find(epsCAV==epsmin);
beta01opt=beta01_v(beta01idx); beta03opt=beta03_v(beta03idx);
wmin=min(min(wCAV));
[beta01lossidx,beta03lossidx]=find(wCAV==wmin);
beta01lossopt=beta01_v(beta01lossidx); beta03lossopt=beta03_v(beta03lossidx);
...
% grid in plane of parameters
[beta01,beta03,om]=ndgrid(beta01_v,beta03_v,om_v);
[~,~,rho]=ndgrid(beta01_v,beta03_v,rho_v);
...
% cost optimum
Jmin=min(min(J));
[beta01costidx,beta03costidx]=find(J==Jmin);
beta01costopt=beta01_v(beta01costidx); beta03costopt=beta03_v(beta03costidx);

%% Plot of energy contours
% plot energy contours without loss terms
figure(11); clf; hold on; box on;
contour(beta01_m,beta03_m,epsCAV,epsminlevel:2:epsmaxlevel);
% ,'r','ShowText','on'
LL=plot(beta01opt,beta03opt,'kx','Linewidth',1.5);
axis([beta01min,beta01max,beta03min,beta03max]);
...
legend(LL,['\epsilon=',num2str(epsmin),' [J/kg]',10,'(\beta_{01},\beta_{03})=(',...
    num2str(beta01opt),',',num2str(beta03opt),')'],...
    'Location','southwest');

% plot energy contours with loss terms
figure(12); clf; hold on; box on;
contour(beta01_m,beta03_m,wCAV,wminlevel:2:wmaxlevel);
% ,'r','ShowText','on'
LL=plot(beta01lossopt,beta03lossopt,'kx','Linewidth',1.5);
axis([beta01min,beta01max,beta03min,beta03max]);
...
legend(LL,['w=',num2str(wmin),' [J/kg]',10,'(\beta_{01},\beta_{03})=(',...
    num2str(beta01lossopt),',',num2str(beta03lossopt),')'],...
    'Location','southwest');
...
% contour(beta01(:,:,1),beta03(:,:,1),Mmax,[1,1],'b','Linewidth',1.5);
contour(beta01(:,:,1),beta03(:,:,1),J,Jminlevel:0.01:Jmaxlevel);
LL=plot(beta01costopt,beta03costopt,'kx','Linewidth',1.5);
axis([beta01min,beta01max,beta03min,beta03max]);
...
legend(LL,['J=',num2str(Jmin),' [m^2/s^4]',10,'(\beta_{01},\beta_{03})=(',...
    num2str(beta01costopt),',',num2str(beta03costopt),')'],...
    'Location','southwest');
...
disp(['Energy consumption with beta01=',num2str(beta01_v(idx1),'%3.2f'),', beta03=',num2str(beta03_v(idx2),'%3.2f'),':']);
...
disp(['Oeps:   epsi=',num2str(epsFollow(beta01idx,beta03idx,end,end),'%4.1f'),' [J/kg]',9,...
          'Despi=',num2str((epsFollow(beta01idx,beta03idx,end,end)-epsL(time(end)))/epsL(time(end))*100,'%4.1f'),' [%]',9,...
          'w=',num2str(wFollow(beta01idx,beta03idx,end,end),'%4.1f'),' [J/kg]',9,...
          'Dw=',num2str((wFollow(beta01idx,beta03idx,end,end)-wL(end))/wL(end)*100,'%4.1f'),' [%]']);
disp(['Ow:	    epsi=',num2str(epsFollow(beta01lossidx,beta03lossidx,end,end),'%4.1f'),' [J/kg]',9,...
          'Despi=',num2str((epsFollow(beta01lossidx,beta03lossidx,end,end)-epsL(time(end)))/epsL(time(end))*100,'%4.1f'),' [%]',9,...
          'w=',num2str(wFollow(beta01lossidx,beta03lossidx,end,end),'%4.1f'),' [J/kg]',9,...
          'Dw=',num2str((wFollow(beta01lossidx,beta03lossidx,end,end)-wL(end))/wL(end)*100,'%4.1f'),' [%]']);
disp(['OJ:     epsi=',num2str(epsFollow(beta01costidx,beta03costidx,end,end),'%4.1f'),' [J/kg]',9,...
          'Despi=',num2str((epsFollow(beta01costidx,beta03costidx,end,end)-epsL(time(end)))/epsL(time(end))*100,'%4.1f'),' [%]',9,...
          'w=',num2str(wFollow(beta01costidx,beta03costidx,end,end),'%4.1f'),' [J/kg]',9,...
          'Dw=',num2str((wFollow(beta01costidx,beta03costidx,end,end)-wL(end))/wL(end)*100,'%4.1f'),' [%]']);