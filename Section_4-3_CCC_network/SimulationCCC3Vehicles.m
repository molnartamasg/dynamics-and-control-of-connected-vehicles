%% Perform simulations for CCC in a 3 vehicle scenario
clear; %close all; clc;
...
% CAV
u0=@(s0,v0,s1,v1,v2)alpha01*(V0(s1-s0-l)-v0)+...
...
parlist=['CHV: \alpha_{\rm h}=',num2str(alphah,'%3.2f'),' [1/s]   ',...
...
model=@(t,x,xdelay,sL,vL,tau,sigma)...
    [x(2);...
     ...
     x(4);...
     sat(u0(xdelay(3,2),xdelay(4,2),xdelay(1,2),xdelay(2,2),vL(t-sigma)))];
